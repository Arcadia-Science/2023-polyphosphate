...
```
conda env create -n polyphosphate -f environment.yml
```
This analysis was developed and run on macOS. The Nextflow comparison step additionally requires Docker (see below).

## Reproducing the analysis
The full analysis runs in the following order. Steps 2–4 and the tree build (step 6 below) are computationally expensive; their key outputs are already committed to this repository (figures in `figs/`, comparison tables in `results/`, and metadata tables in `metadata/`), so the plotting scripts can be inspected against the committed results without re-running the upstream steps.

1. **Filter accessions and build metadata** — `scripts/ppk1-uniprot-accessions-filtering.R` reads the raw Uniprot accession exports in `metadata/` and writes `metadata/all-filtered-ppk1-accessions.tsv` and `metadata/filtered-ppk1-accessions-list.txt`.
2. **Download sequences and structures** — `scripts/download_alphafold_pdbs.py` populates `protein_structures/` (see "Prep and Download Accessions").
3. **Cluster all structures** — the ProteinCartography `from-folder` workflow (see "Cluster all proteins") produces the PCA embeddings `results/ppk1_clustering_results/all_by_all_tmscore_pivoted_pca_tsne.tsv` and `..._pca_umap.tsv`, plus `struclusters_features.tsv` and `leiden_features.tsv`.
4. **Sequence/structure comparison to the query** — `nextflow run main.nf ...` (see "Workflow for `mmseqs` and `foldseek` comparisons") produces `results/A0A369XMZ4_CAP_ppk1_sequences_search.m8` and `..._structures_search.m8`.
5. **Plot comparisons and clusters** — `scripts/ppk1-seq-vs-structure-comps.R` and `scripts/cluster-plotting.R` read the committed `.m8` files, the ProteinCartography clustering TSVs, and the metadata table, and write the figures in `figs/` and the cluster tables in `results/`.
6. **Phylogeny and phylogenetic-distance comparisons** — build the tree (see "Phylogeny of _Pseudomonadota_ Ppk1 Sequences") to produce `results/ppk1_trees/2023-08-15-itol.tre.txt`, then run `scripts/phylo-comps.R`.

### Regenerable intermediate files (not committed)
A few large intermediates are `.gitignore`d and must be regenerated by the step noted above before the corresponding script can be re-run end to end:
- `protein_structures/` (sequences + AlphaFold structures) — produced by step 2.
- `results/ppk1_clustering_results/all_by_all_tmscore_pivoted_pca_{tsne,umap}.tsv` — produced by step 3. Their rendered figures (`figs/ppk1_pca_{tsne,umap}_plot_full.*`) are already committed.
- `results/ppk1_trees/2023-08-15-itol.tre.txt` — produced by step 6.
...
The script takes two inputs: `metadata/filtered-ppk1-accessions-list.txt` (one Uniprot accession per line, from step 1) and the AlphaFold accessions CSV. The CSV is the accession mapping obtained from the [AlphaFold DB downloads](https://alphafold.ebi.ac.uk/download); the script reads it with `csv.reader` and expects the Uniprot accession in column 0 (`row[0]`) and the corresponding AlphaFold accession in column 3 (`row[3]`), where AlphaFold accessions take the form `AF-<uniprot>-F1`. This CSV is not committed to the repository. Structures are saved as `<uniprot>-F1-model_v4.pdb` in the structures directory. Run it, for example, with:
```
python scripts/download_alphafold_pdbs.py metadata/filtered-ppk1-accessions-list.txt <alphafold_accessions.csv> \
    --proteins_directory protein_structures/fastas \
    --structures_directory protein_structures/structures
```
...
To reproduce this step, install ProteinCartography at the commit used here and follow its own environment-setup instructions:
```
git clone https://github.com/Arcadia-Science/ProteinCartography.git
cd ProteinCartography
git checkout bc7f2059eae4cbebe9edce6364c5650c1873f51e
```
Then, from the ProteinCartography directory, run the pipeline with:
```
